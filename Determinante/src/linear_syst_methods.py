...
def eliminacion_gaussiana(A: np.ndarray) -> np.ndarray:
   ...
   # --- Sustitución hacia atrás
    print("Matriz antes de la sustitución hacia atrás:")
    print(A)
    solucion = np.zeros(n)
...
def eliminacion_gaussiana_det(A: np.ndarray) -> np.ndarray:
    """Realiza la eliminación gaussiana en una matriz cuadrada A.
    ...
    ``A``: matriz triangular superior obtenida después de la eliminación gaussiana, con valores redondeados a 2 decimales.
    """
    if not isinstance(A, np.ndarray):
        logging.debug("Convirtiendo A a numpy array.")
        A = np.array(A)
    assert A.shape[0] == A.shape[1], "La matriz A debe ser cuadrada (nxn)."
    n = A.shape[0]
    ...
    for i in range(0, n - 1):  # loop por columna
        ...
        for pi in range(i, n):
            ...
            if p is None:
                # primer elemento distinto de cero
                p = pi
                continue
    ...
    # Redondear la matriz triangular superior a 2 decimales
    A_redondeada = np.round(A, 2)

    # Imprimir solo la matriz triangular superior al final
    print("Matriz triangular superior después de la eliminación gaussiana:")
    print(A_redondeada)
